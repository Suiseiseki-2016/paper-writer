    ...
    def _generate_paper_content(self, paper: PaperBase) -> PaperBase:
        ...
        for i in range(len(paper.section_content)):
            ...
            while True:
                ...
                if index1 == -1:
                    break
                print(content[index2-1])
                content = content[0:index1] + paper.citation_content[i][int(content[index2-1]) - 1].citation_sentence + \
        ...
        # 生成Reference
        reference_index = 1
        paper.paper_content = paper.paper_content + '\nReferences\n'
        
        for section_references in paper.references:
            for reference in section_references:
                paper.paper_content = paper.paper_content + f'[{reference_index}] ' + reference.reference
                reference_index += 1
